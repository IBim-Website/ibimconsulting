...
export async function POST(request) {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
            ...
            if (customerEmail) {
                ...
                if (!snapshot.empty) {
                    ...
                    if (aiData.insights) {
                        ...
                        console.log(`✅ Doc ${latestDoc.id} updated with AI insights.`);
                    }
        ...
        // This is the "magic" line that prevents the function from timing out early
        if (request.waitUntil) {
        request.waitUntil(backgroundTask);
        }

        // 3. Return immediately to Stripe
        // Stripe gets a 200 OK within milliseconds, preventing retries
